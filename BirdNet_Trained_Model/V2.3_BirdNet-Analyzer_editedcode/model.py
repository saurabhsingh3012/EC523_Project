...
def trainLinearClassifier(classifier, x_train, y_train, epochs, batch_size, learning_rate):
    ...
    classifier.compile(optimizer=keras.optimizers.Adam(learning_rate=lr_schedule), 
                       ...
                       metrics=[keras.metrics.Precision(top_k=1, name='prec'),keras.metrics.FBetaScore(average=None,beta=0.5,threshold=None,name='fbeta_score',dtype=None)]
                        )
    ...
    fbeta_score = history.history['val_fbeta_score'][np.argmin(history.history['val_loss'])]
    #mean_average_precision = history.history['val_mean_average_precision'][np.argmin(history.history['val_loss'])]


    return classifier, best_val_prec, fbeta_score
